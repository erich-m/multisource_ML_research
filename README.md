...
- [Usage](#usage)
- [Models, Performance, Results](#models-performance-results)
...
3. Next, each of the encounters that are extracted have the eye tracking and the driving data merged together. First the datasets are aligned in time based on offsets found in `data_summary.xlsx`, and then they are merged together. Next, the missing values from encoutner extractions are filled using spline interpolation with a default spline order of 5 to fill in the gaps. The merged encounters are then stored in `encounter_data`
Some of the eye tracking data is completely missing likely due to technical issues during data collection?....these rows are removed at the end since much of the processing stages require all participants to be intact
