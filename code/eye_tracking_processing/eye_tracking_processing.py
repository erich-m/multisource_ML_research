...
for summary_index, summary_row in tqdm(data_summary.iterrows(), total=len(data_summary)):
    ...
    gaze_encounter = pd.read_csv(f'intermediary_data/extracted_encounters_5/participant_{p_num}/gaze_encounter_{p_num}.csv')
    ...
    for start in gap_starts:
        ...
        if len(gap_end_candidates) > 0:
                ...
                for col in columns_to_interpolate:
                    ...
                    if last_valid is not None and next_valid is not None:
                        ...
                        interpolated_values = np.interp(
                            processed_gaze.loc[last_valid:next_valid, 'Timestamp'],
                            [processed_gaze.loc[last_valid, 'Timestamp'], processed_gaze.loc[next_valid, 'Timestamp']],
                            [processed_gaze.loc[last_valid, col], processed_gaze.loc[next_valid, col]]
                        )
            ...
            pass
            # print(f"No valid end found for gap starting at index {start}")
    ...
    for i in range(window_length_samples, len(processed_gaze) - window_length_samples):
        ...
        if time_difference != 0:
            processed_gaze.loc[i, 'Velocity'] = visual_angle / time_difference
        else:
    ...
    processed_gaze.to_csv(f'intermediary_data/processed_eye_tracking/participant_{p_num}/processed_gaze_{p_num}.csv', index=False)
